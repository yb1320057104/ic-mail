...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("GET /api/icloud/auto-login/logs", s.handleAutoLoginLogs)
	...
	s.mux.HandleFunc("POST /api/proxy-pool/import", s.handleImportProxyPool)
...
func (s *Server) handlePublicRedeem(w http.ResponseWriter, r *http.Request) {
	...
	var p struct {
		...
		APIType        string   `json:"api_type"`
		LookupPassword string   `json:"lookup_password"`
	}
	if err := decodeJSON(r, &p); err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
	if n := len([]rune(strings.TrimSpace(p.LookupPassword))); n < 4 || n > 64 {
		writeError(w, http.StatusBadRequest, errCode("invalid_lookup_password", "请设置 4 到 64 个字符的查单密码", false))
		return
	}
	pool, ok := s.store.RedemptionPoolByToken(token)
	if !ok {
...
func (s *Server) handlePublicRedemptionOrders(w http.ResponseWriter, r *http.Request) {
	var p struct {
	...
	orders, boxes, err := s.store.RedemptionOrdersByPassword(r.PathValue("token"), p.Password)
	if err != nil {
...
func (s *Server) handleManageData(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"accounts":        accounts,
		"mailboxes":       mailboxes,
		"mailbox_count":   len(state.Mailboxes),
...
func (s *Server) handleCheckICloudSession(w http.ResponseWriter, r *http.Request) {
	...
	var lastErr error
	for _, session := range sessions {
		client, clientErr := s.iCloudClientForAccount(r.Context(), ownerID, session.AccountID)
		if clientErr != nil {
			failed++
			lastErr = clientErr
			continue
...
func (s *Server) handleCheckICloudWebLogin(w http.ResponseWriter, r *http.Request) {
	...
	client, clientErr := s.iCloudClientForAccount(checkCtx, ownerID, session.AccountID)
	var err error
	if clientErr != nil {
		err = clientErr
	} else {
...
func (s *Server) handleSaveICloudIMAPLogin(w http.ResponseWriter, r *http.Request) {
	...
	proxyState := LoginState{IMAPEmail: email, IMAPUsername: imapUsername, IMAPHost: imapHost, IMAPPort: imapPort, IMAPAppPassword: appPassword, ProxyURL: proxyURL}
	if strings.EqualFold(imapHost, defaultICloudIMAPHost) && imapPort == defaultICloudIMAPPort && imapUsername == email {
		if proxyURL != "" {
			checkErr = s.checkSavedIMAPState(r.Context(), proxyState)
		} else {
...
func (s *Server) handleCheckICloudIMAPLogin(w http.ResponseWriter, r *http.Request) {
	...
	for _, session := range sessions {
		...
		checks++
		if proxyURL, _, err := s.proxyURLForAccount(r.Context(), ownerID, session.AccountID); err != nil {
			lastErr = err
			failed++
			continue
		} else {
...
func (s *Server) checkSavedIMAPState(ctx context.Context, state LoginState) error {
	...
	if state.ProxyURL != "" {
		...
		if err != nil {
			return errCode("imap_connect_failed", "通过账号代理连接 IMAP 失败："+err.Error(), true)
		}
		defer conn.Close()
		reader := bufio.NewReader(conn)
		greeting, err := reader.ReadString('\n')
		if err != nil || !strings.Contains(strings.ToUpper(greeting), "OK") {
			return errCode("imap_greeting_failed", "IMAP 未就绪", true)
		}
		lines, err := imapCommand(conn, reader, "A001", "LOGIN "+imapQuote(username)+" "+imapQuote(state.IMAPAppPassword))
		if err != nil || !imapTaggedOK(lines, "A001") {
			return errCode("imap_login_failed", "IMAP 登录失败："+imapResponseSummary(lines), false)
		}
		_, _ = imapCommand(conn, reader, "A002", "LOGOUT")
		return nil
	}
...
func (s *Server) handleSubmitICloudProtocol2FA(w http.ResponseWriter, r *http.Request) {
	...
	ownerID := requestOwnerID(r, s.store)
	authClient, err := s.appleAuthClientForLogin(r.Context(), ownerID, "", pending.Session.AppleID)
	if err != nil {
		writeError(w, http.StatusBadGateway, err)
		return
	}
...
func (s *Server) syncICloudMailboxesForSession(ctx context.Context, r *http.Request, ownerID string, session ICloudSession) (syncICloudMailboxResult, []publicMailbox, error) {
	...
	client, clientErr := s.iCloudClientForAccount(ctx, ownerID, session.AccountID)
	if clientErr != nil {
...
func (s *Server) handleCleanRemoteMailbox(w http.ResponseWriter, r *http.Request) {
	...
	client, clientErr := s.iCloudClientForAccount(r.Context(), mailbox.OwnerID, mailbox.AccountID)
	if clientErr != nil {
...
func (s *Server) handleMailboxContentByEmail(w http.ResponseWriter, r *http.Request) {
	...
	done := s.startMailboxContentSync(mailbox, mailboxCodeSyncAfter(cutoff, time.Now()))
...
func mailboxCodeSyncAfter(after, now time.Time) time.Time {
	if now.IsZero() {
		now = time.Now()
	}
	lookback := now.Add(-mailboxCodeSyncLookback)
	if after.IsZero() || after.After(lookback) {
		return lookback
	}
	return after
}
...
func mailboxMessageAvailableAt(message Message) time.Time {
	receivedAt := message.ReceivedAt
	createdAt := message.CreatedAt
	if receivedAt.IsZero() {
		return createdAt
	}
	if createdAt.IsZero() || !createdAt.After(receivedAt) {
		return receivedAt
	}
	// Most email Date headers only have second or minute precision. Use the
	// local ingestion time for a newly arrived message, but never make an old
	// historical message fresh merely because it was imported just now.
	if createdAt.Sub(receivedAt) <= mailboxReceivedTimePrecisionAllowance {
		return createdAt
	}
	return receivedAt
}
...
func (s *Server) syncMailboxesForCodeWaiters(ctx context.Context, ownerID string, waiters []*mailboxCodeWaiter) error {
	...
	for _, group := range groups {
		...
		syncAfter := mailboxCodeSyncAfter(group.minAfter, time.Now())
		_, err := s.syncMailboxCodeBatchForOwnerWithLimit(ctx, ownerID, mailboxes, syncAfter, group.keyword, mailboxCodeRequestFetchLimit)
...
func (s *Server) keepAliveICloudWebRoundWithForce(ctx context.Context, force bool) {
	...
	for _, session := range s.iCloudWebKeepAliveSessions() {
		...
		go func(session ICloudSession, state LoginState) {
			...
			metricStarted := time.Now()
			client, clientErr := s.iCloudClientForAccount(callCtx, session.OwnerID, session.AccountID)
			var err error
			if clientErr != nil {
				err = clientErr
			} else {
...
func (s *Server) syncMailboxCodeBatchForOwnerWithLimit(ctx context.Context, ownerID string, mailboxes []Mailbox, after time.Time, keyword string, maxMessages int) (int, error) {
	...
	for _, mailbox := range refreshed {
		...
		if proxyURL, _, err := s.proxyURLForAccount(ctx, ownerID, session.AccountID); err != nil {
			return 0, err
		} else {
			state.ProxyURL = proxyURL
		}
		key := imapStateKey(state) + "|" + state.ProxyURL
		group := groups[key]
		if group == nil {
...
func (s *Server) allowsUserSession(r *http.Request) bool {
	...
	if strings.HasPrefix(r.URL.Path, "/api/user/proxy-pool/") || r.URL.Path == "/api/user/proxy-pool" {
		return true
	}
	if strings.HasPrefix(r.URL.Path, "/api/proxy-pool/") || r.URL.Path == "/api/proxy-pool" {
		return true
	}
